...
export default function TestimonialsSection() {
  ...
  useEffect(() => {
    axios
      .get(`${API_URL}/api/feedback/public-stats`, { params: { days: 90 } })
      .then((res) => {
        const data = res.data || {};
        if (data.show_public) {
  ...
  return (
    <section className="py-20 sm:py-28 bg-muted/50 overflow-hidden">
      ...
      <div className="train-scroll">
        ...
        >
          {doubled.map((t, i) => (
            <div
              key={i}
              className="w-[300px] sm:w-[340px] flex-shrink-0 bg-card rounded-2xl border border-border p-5 shadow-sm"
            >

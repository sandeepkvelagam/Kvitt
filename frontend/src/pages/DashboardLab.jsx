...
import {
  ...
  Wallet, Target, Crown, CalendarDays, Bell, Sparkles, Zap,
      ...
      <div className="lab-bg text-slate-100">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-4 gap-3">
            <div className="col-span-2 row-span-2"><GlassTileSkeleton size="hero" className="h-full min-h-[240px]" /></div>
            <GlassTileSkeleton size="md" className="min-h-[114px]" />
            <GlassTileSkeleton size="md" className="min-h-[114px]" />
            <GlassTileSkeleton size="md" className="min-h-[114px]" />
            <GlassTileSkeleton size="md" className="min-h-[114px]" />
            <div className="col-span-2"><GlassTileSkeleton size="lg" className="min-h-[200px]" /></div>
            <div className="col-span-2"><GlassTileSkeleton size="lg" className="min-h-[200px]" /></div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="lab-bg text-slate-100">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden" aria-hidden="true">
        ...
        <div className="absolute top-[60%] right-[30%] w-[300px] h-[300px] rounded-full opacity-[0.04]"
      ...
      <main className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div style={stagger(0)} className="mb-5">
          <button
            ...
            className="text-xs px-3 py-1.5 rounded-full glass-chip cursor-pointer text-slate-400 hover:text-orange-400 font-dot"
          >
            &larr; BACK
          </button>
        </div>
        ...
        <div style={stagger(1)} className="mb-8">
          ...
          <p className="font-dot text-slate-500 mt-2 text-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          {/* ---- TILE 1: Net Profit (tall, 2 cols, 2 rows) ---- */}
          <div style={stagger(2)} className="col-span-2 row-span-2">
            <GlassTile size="hero" tone="purple" elevated onClick={() => navigate("/history")} className="h-full">
              <div className="flex flex-col h-full justify-between min-h-[220px]">
                <div>
                  <p className="font-dot text-[11px] tracking-[0.2em] uppercase text-purple-300/60 mb-1">
                    ....NET.PROFIT....
                  </p>
                  <div className="flex items-start justify-between">
                    <p className={`font-pixel text-5xl sm:text-6xl font-bold tracking-tight ${netProfit >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                  ...
                  <p className="font-dot text-[11px] text-slate-500 mt-2">
                ...
                <div className="grid grid-cols-4 gap-2 mt-4">
                  {[
                    ...
                    <div key={s.label} className="text-center p-2 rounded-xl bg-white/[0.04] border border-white/[0.05]">
                      <p className={`font-pixel text-sm font-bold ${s.color}`}>{s.value}</p>
                      <p className="font-dot text-[9px] text-slate-600 mt-0.5">{s.label}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-3 flex items-center gap-2">
                  <span className="font-dot text-[10px] text-slate-600 w-8">ROI</span>
                  ...
                  <span className={`font-pixel text-xs font-bold ${roiPercent >= 0 ? "text-emerald-400" : "text-red-400"}`}>
          ...
          <div style={stagger(3)}>
            <GlassTile size="md" tone="blue" elevated className="h-full">
              ...
              <div className="flex items-end gap-1">
                <p className="font-pixel text-4xl font-bold text-slate-100">{winRate.toFixed(0)}</p>
                <p className="font-pixel text-xl text-slate-500 mb-1">%</p>
              </div>
              <div className="flex items-center gap-1 mt-2">
                <div className="w-6 h-6 rounded-lg bg-blue-500/15 flex items-center justify-center">
                  <Target className="w-3 h-3 text-blue-400" />
                </div>
                <span className="font-dot text-[10px] text-slate-600">
          ...
          <div style={stagger(4)}>
            <GlassTile size="md" tone="mint" elevated onClick={() => navigate("/wallet")} className="h-full">
              ...
              <p className={`font-pixel text-3xl font-bold ${netBalance >= 0 ? "text-emerald-400" : "text-red-400"}`}>
          ...
          <div style={stagger(5)}>
            <GlassTile size="md" tone="amber" elevated onClick={() => navigate("/schedule")} className="h-full">
              ...
              {activeGames.length > 0 ? (
                <>
                  <p className="font-dot text-sm text-slate-200 truncate mb-1">
                    {activeGames[0].title || activeGames[0].group_name}
                  </p>
                  <div className="flex items-center gap-2">
                    <span className={`text-[9px] font-dot px-2 py-0.5 rounded-full ${activeGames[0].status === "active" ? "bg-emerald-500/15 text-emerald-400" : "bg-amber-500/15 text-amber-400"}`}>
                      {activeGames[0].status === "active" ? "LIVE" : "SOON"}
                    </span>
                    <span className="font-dot text-[10px] text-slate-600">
                      {activeGames[0].player_count || 0}P
                    </span>
                  </div>
                </>
              ) : (
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="font-dot text-[11px] text-slate-600">NO.UPCOMING</span>
                </div>
              )}
            </GlassTile>
          </div>

          {/* ---- TILE 5: Quick Stats (square, 1 col) ---- */}
          <div style={stagger(6)}>
            <GlassTile size="md" tone="rose" elevated className="h-full">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-dot text-[10px] text-slate-500">ACTIVE</span>
                  <span className="font-pixel text-lg font-bold text-emerald-400">{activeGames.length}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="font-dot text-[10px] text-slate-500">GROUPS</span>
                  <span className="font-pixel text-lg font-bold text-blue-400">{groups.length}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="font-dot text-[10px] text-slate-500">TOTAL</span>
                  <span className="font-pixel text-lg font-bold text-amber-400">{totalGames}</span>
                </div>
              </div>
            </GlassTile>
          </div>

          {/* ---- TILE 6: Live Games (tall, 2 cols, 2 rows) ---- */}
          <div style={stagger(7)} className="col-span-2 row-span-2">
            <GlassTile size="lg" tone="mint" className="h-full">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-dot text-[11px] tracking-[0.15em] uppercase text-emerald-300/50">LIVE.GAMES</span>
                </div>
                <span className="font-dot text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                  {activeGames.length}
                </span>
              </div>
              ...
              {activeGames.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="font-dot text-[12px] text-slate-600">NO.ACTIVE.GAMES</p>
                  <p className="font-dot text-[10px] text-slate-700 mt-1">START.FROM.YOUR.GROUP</p>
                </div>
              ) : (
                <div className="space-y-2">
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <p className="font-dot text-[12px] text-slate-200 truncate">{game.title || game.group_name}</p>
                        </div>
                        <span className={`font-dot text-[9px] px-1.5 py-0.5 rounded-full ${game.status === "active" ? "bg-emerald-500/10 text-emerald-400" : "bg-amber-500/10 text-amber-400"}`}>
                          {game.status === "active" ? "LIVE" : "SOON"}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 font-dot text-[10px] text-slate-600 ml-3.5">
                        <Crown className="w-2.5 h-2.5 text-amber-500/60" />
                        <span>{game.host_name || "Host"}</span>
                        <span>&middot;</span>
                        <span>{game.player_count || 0}P</span>
                        <span>&middot;</span>
              ...
              <button
                className="w-full mt-3 py-2 rounded-xl font-dot text-[11px] tracking-wider text-slate-500 border border-white/[0.06] hover:border-emerald-500/20 hover:text-emerald-400 transition-all cursor-pointer bg-transparent"
          ...
          {/* ---- TILE 7: Groups (wide, 2 cols) ---- */}
          <div style={stagger(8)} className="col-span-2">
            <GlassTile size="lg" tone="blue" className="h-full">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-dot text-[11px] tracking-[0.15em] uppercase text-blue-300/50">MY.GROUPS</span>
                </div>
                <span className="font-dot text-[10px] px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20">
                  {groups.length}
                </span>
              </div>
              ...
              {groups.length === 0 ? (
                <div className="text-center py-4">
                  <p className="font-dot text-[12px] text-slate-600">NO.GROUPS.YET</p>
                </div>
              ) : (
                  ...
                  {groups.slice(0, 3).map(group => (
                    ...
                    >
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                        ...
                        <span className="font-pixel text-sm font-bold text-orange-400">
                          {group.name?.[0]?.toUpperCase() || "G"}
                        </span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-dot text-[12px] text-slate-200 truncate">{group.name}</p>
                        <p className="font-dot text-[10px] text-slate-600">{group.member_count} MEMBERS</p>
                      </div>
                      {group.user_role === "admin" && (
                        <span className="font-dot text-[9px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20">
                          ADMIN
                        </span>
                      )}
              ...
              <button
                className="w-full mt-3 py-2 rounded-xl font-dot text-[11px] font-medium text-white border-0 cursor-pointer flex items-center justify-center gap-2 transition-all"
          ...
          {/* ---- TILE 8: Recent Results (wide, 2 cols) ---- */}
          <div style={stagger(9)} className="col-span-2">
            <GlassTile size="md" tone="slate">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-7 h-7 rounded-lg bg-violet-500/10 flex items-center justify-center">
                  <Clock className="w-3.5 h-3.5 text-violet-400" />
                </div>
                <span className="font-dot text-[11px] tracking-[0.15em] uppercase text-violet-300/50">RECENT.RESULTS</span>
              </div>
              {stats?.recent_games?.length > 0 ? (
                    ...
                    <div key={idx} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/[0.02] transition-colors">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="font-dot text-[12px] text-slate-300 truncate">{game.group_name}</p>
                          <p className="font-dot text-[10px] text-slate-600">
                            {game.date ? new Date(game.date).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "Recent"}
                          </p>
                        </div>
                      </div>
                      <span className={`font-pixel text-sm font-bold flex-shrink-0 ${game.net_result >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                ...
                <p className="font-dot text-[12px] text-slate-600 text-center py-4">NO.RECENT.GAMES</p>
              )}
            </GlassTile>
          </div>

          {/* ---- TILE 9: AI Insights (1 col) ---- */}
          <div style={stagger(10)}>
            <GlassTile size="md" tone="purple" elevated onClick={() => navigate("/ai")} className="h-full">
              <div className="flex items-center gap-2 mb-2">
                <Bot className="w-4 h-4 text-purple-400" />
                <span className="font-dot text-[10px] tracking-[0.15em] text-purple-300/50">AI.INSIGHT</span>
              </div>
              <p className="font-dot text-[11px] text-slate-400 leading-relaxed">
                {totalGames > 5
                  ? `${winRate.toFixed(0)}% WIN RATE. ${winRate > 50 ? "HOT.STREAK!" : "LET'S.ANALYZE."}`
                  : "PLAY.MORE.TO.UNLOCK.AI.INSIGHTS"}
              </p>
              <div className="mt-2 flex items-center gap-1 font-dot text-[10px] text-purple-400">
                <Sparkles className="w-3 h-3" /> VIEW &rarr;
              </div>
            </GlassTile>
          </div>

          {/* ---- TILE 10: Quick Actions (1 col) ---- */}
          <div style={stagger(11)}>
            <GlassTile size="md" tone="orange" elevated onClick={() => navigate("/groups")} className="h-full">
              ...
              <p className="font-dot text-[12px] text-slate-200">START.GAME</p>
              <p className="font-dot text-[10px] text-slate-600 mt-0.5">QUICK.LAUNCH</p>
            </GlassTile>
          </div>

          {/* ---- TILE 11: Wallet Quick (wide, 2 cols) ---- */}
          <div style={stagger(12)} className="col-span-2">
            <GlassTile size="sm" tone="mint" elevated onClick={() => navigate("/wallet")}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Wallet className="w-5 h-5 text-emerald-400" />
                  <div>
                    <p className="font-dot text-[12px] text-slate-200">WALLET</p>
                    <p className="font-dot text-[10px] text-slate-600">MANAGE.FUNDS.&amp;.SETTLEMENTS</p>
                  </div>
                </div>
